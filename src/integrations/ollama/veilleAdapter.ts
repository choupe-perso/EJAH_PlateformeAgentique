...
async function appelSecurise<T>(prompt: string, mapper: (texte: string) => T): Promise<MoteurResultat<T>> {
  try {
    const reponse = await appelOllama(prompt);
...
export const ollamaDialogue: MoteurDialogue = {
  id: "ollama",
  converser({ contexte, historique, messageUtilisateur }) {
    const prompt = `${contexte}\n\n${formaterHistorique(historique)}\nUtilisateur: ${messageUtilisateur}\nAssistant:`;
    return appelSecurise(prompt, (reponse) => ({ reponse: reponse.trim() }));
  },
};

export const ollamaAnalyse: MoteurAnalyse = {
  id: "ollama",
  extraireObservations({ contexte, contenu }) {
    const prompt = `${contexte}\n\nContenu a analyser :\n${contenu}\n\nReponds uniquement avec un tableau JSON d'observations.`;
    return appelSecurise(prompt, (reponse) => ({
      observations: extraireJson<Array<Record<string, unknown>>>(reponse) ?? [],
    }));
  },
