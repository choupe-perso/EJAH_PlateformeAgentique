...
def test_manifest_declares_mandatory_cli_capability() -> None:
    ...
    assert manifest["agent"]["id"] == "anonymizer"
    ...
    assert manifest["access_control"]["gate_question"] == "Êtes-vous un(e) consultant(e) ADBI ? [O/N]"
...
def test_contract_describes_inputs_and_outputs() -> None:
    ...
    assert "documents" in contract["input"]["required"]
    assert contract["output"]["required"] == ["message", "results"]
