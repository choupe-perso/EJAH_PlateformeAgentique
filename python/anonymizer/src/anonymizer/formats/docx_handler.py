...
Text inside a hyperlink (<w:hyperlink><w:r>...) and text inside a text box
(<w:txbxContent>, DrawingML shape or legacy VML) are both reached too -
neither is visible via plain `paragraph.runs` (a python-docx limitation:
it only looks at <w:r> elements that are direct children of <w:p>), so
`_all_runs_by_paragraph` below collects them explicitly instead of relying
on `paragraph.runs`.

Known coverage gaps still open, documented rather than silently swallowed:
...
- A logo/graphic whose text OCR fails to read reliably (small, stylised,
  low-contrast) won't be flagged - this is an OCR accuracy limit, not a
  traversal gap: the image itself is scanned like any other.
"""
